...
- **`contrib.chat_provider.openai_legacy._convert_message`** — for strict-interleaved providers (`kimi-k2*`, `deepseek*`) now **always** emits `reasoning_content` on assistant turns, even when no `ThinkPart` was captured. Falls back to the assistant text, then to `"[reasoning unavailable]"` so Moonshot/Kimi's "thinking is enabled but reasoning_content is missing in assistant tool call message at index N" rejection no longer trips multi-step tool flows. Fixes pythinker-code [#37](https://github.com/mohamed-elkholy95/Pythinker-Code/issues/37).
